...
contract EggTracker is IEggTracker {
    ...
    // 事件
    event EggRegistered(uint256 indexed eggId, address indexed farmAddress, uint256 productionDate);
    event EggInfoUpdated(uint256 indexed eggId, address indexed farmAddress);
    
    // 修饰器：仅认证农场可调用
    modifier onlyRegisteredFarm() {
        require(IAuthCenter(authCenterAddress).isRegisteredFarm(msg.sender), "Caller is not a registered farm");
        _;
    }
    
    // 构造函数
    constructor(address _authCenterAddress) {
        require(_authCenterAddress != address(0), "Auth center address cannot be zero");
    ...
    function setAuthCenterAddress(address _authCenterAddress) external {
        require(msg.sender == authCenterAddress, "Only current auth center can change address");
        require(_authCenterAddress != address(0), "Auth center address cannot be zero");
        authCenterAddress = _authCenterAddress;
    }
        ...
        onlyRegisteredFarm 
    {
        ...
        require(eggs[_eggId].farmAddress == msg.sender, "Only egg's farm can update info");
        require(_expiryDate > eggs[_eggId].productionDate, "Expiry date must be after production date");
     ...
     */
    function getEggDetails(uint256 _eggId) 
        external 
        view 
        override
        returns (
        ...
        ) 
    {
        ...
        // 从认证中心获取农场信息
        (farmName, farmLocation, ) = IAuthCenter(authCenterAddress).getFarmDetails(egg.farmAddress);
     ...
     * @return true 如果鸡蛋已过期
     */
    function isEggExpired(uint256 _eggId) external view override returns (bool) {
        require(eggs[_eggId].isRegistered, "Egg not registered");
     ...
     */
    function getFarmEggs(address _farmAddress) external view returns (uint256[] memory) {
        require(IAuthCenter(authCenterAddress).isRegisteredFarm(_farmAddress), "Not a registered farm");
     ...
     */
    function isAuthenticEgg(uint256 _eggId) external view override returns (bool) {
        if (!eggs[_eggId].isRegistered) {
            return false;
        }
        
        return IAuthCenter(authCenterAddress).isRegisteredFarm(eggs[_eggId].farmAddress);
    }
}
